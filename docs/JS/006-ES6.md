...
- 其次，WeakSet 中的对象都是弱引用，即**垃圾回收机制不考虑 WeakSet 对该对象的引用**，也就是说，如果其他对象都不再引用该对象，那么垃圾回收机制会自动回收该对象所占用的内存，不考虑该对象还存在于 WeakSet 之中。
...
| values()        | keys`方法和`values`方法的行为完全一致 |`values()`：返回键值的遍历器 |`values()`返回键值的遍历器                        |
...
**`await`后的语句如同 Promise.then() 语句一样，都在下一个微任务进程中**
