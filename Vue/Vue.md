...
- 组件化, 只要有能够拼接文件的构建系统， 就把每个组件单独分成文件
- 单文件组件文件的大小写，推荐**单词大写开头(PascalCase)**， 或者**始终是横线连接(kebab-case)**
...
- 自闭合组件, 仅限进入DOM之前Vue的模版编译器能够触达的地方
...
- 带引号的特性值
- 指令缩写， 用 **`:`** 表示 **`v-bind：`** , 用`#`表示**`v-slot`**，和用 **`@`** 表示 **`v-on:`** 应该要么都用，要么都不用
...
- `v-html`, 会覆盖原有的内容，会解析HTML
- `v-text`, 会覆盖原有的内容，也不会解析HTML
- `v-bind`, 用于 HTML 特性， 简写**`:href`**, 单向数据传递
- `v-on`， 用户监听 DOM 事件， 简写**`@click`**, 赋值时必须赋值一个回调函数的名称
...
- **`.capture`**,  捕获阶段， 即元素自身触发的事件先在此处理，然后交由内部元素处理
